    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::InvalidArguments => {
                formatter.write_str("usage: grillforge selector [--config-dir PATH]")
            }
...
pub fn run_cli(args: impl IntoIterator<Item = OsString>) -> Result<Option<String>, CliError> {
    ...
    let config_dir = match args.as_slice() {
        ...
        [_, flag, path] if flag == "--config-dir" && !path.is_empty() => PathBuf::from(path),
